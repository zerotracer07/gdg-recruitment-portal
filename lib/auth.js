...
export const auth = betterAuth({
  ...
  // Production: only VIT emails may register (email signup).
  // NOTE: Google OAuth accounts are gated at application time in submit-form.
  hooks: {
      ...
      if (ctx.path === "/sign-up/email") {
        ...
        if (!allowed) {
          throw new APIError("BAD_REQUEST", {
            message: `Please sign up with your VIT email (${ALLOWED_EMAIL_DOMAINS.join(" or ")}).`,
          });
        }
      }
    }),
  ...
  plugins: [
    ...
    // NOTE: signup OTP uses the custom /api/auth/send-code + verify-code
    // endpoints (lib/otp.js). better-auth's emailOTP plugin was removed:
    // its email-verification check requires an existing user, so it cannot
